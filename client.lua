...
local default_server = "wss://skynet.osmarks.net/connect/"
...
function skynet.connect(force, server_url)
	if not skynet.socket or force then
		-- If we already have a socket and are throwing it away, close old one.
		local url = server_url or default_server
		if skynet.socket then skynet.socket.close() end
		local sock = http.websocket(url)
		if not sock then
			return false, "Skynet server unavailable, broken or running newer protocol version."
		end
		skynet.server = url
		skynet.socket = sock
		...
		return true, "Connected successfully to '" .. tostring(url) .. "'."
	end
end
